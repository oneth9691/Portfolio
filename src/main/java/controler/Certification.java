...
public class Certification extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		DupleDTO dto = new DupleDTO();
		
		if(dto.duple_check_email(request.getParameter("email"))==true)
		{
		...
		int port=465; // 메일 내용
		String recipient = request.getParameter("email"); //메일을 발송할 이메일 주소를 기재해 줍니다. 
		String subject = "'출책을 합시당' 회원가입 인증번호"; 
		String body = "인증번호:" + a; 
